...
A comprehensive Learning Management System for courses and workshops.

This is a domain namespace — each LMS module becomes its own Django app
inside this package (e.g. `apps.lms_platform.courses`,
`apps.lms_platform.workshops`), with its own `models.py`,
`serializers.py`, `views.py`, and `urls.py`.

To add an app here:

1. Create the app package with an `AppConfig` where
   `name = "apps.lms_platform.<module>"`.
